...
#include <EEPROM.h>
#include <Wire.h>
#include "BOLIDE_Crawler_Board.h"
#include "Crawler_Mask_Definition.h"
...
//======超音波======//
//port8:前方,port6:左方,port4:右方
MeUltrasonicSensor ultraF8(PORT_8); //前面
MeUltrasonicSensor ultraF6(PORT_6); //左邊
MeUltrasonicSensor ultraF4(PORT_4); //右邊

int ultraFforward = 0 , ultraFleft = 0 , ultraFright = 0;
int mindis = 15; //感測器min dis
int maxdis = 20; //感測器max dis
int sequence_number;//序列埠給值
...
void loop() {
  //==============0817:超音波避障+感測序列埠值動作============//
  ultraFforward = ultraF8.distanceCm(); //前面
  ultraFleft = ultraF6.distanceCm(); //左邊
  ultraFright = ultraF4.distanceCm(); //前面
  Serial.print("forwardD : ");
  Serial.print(ultraFforward.distanceCm() );
  Serial.println(" cm");
  Serial.print("leftD : ");
  Serial.print(ultraFleft.distanceCm() );
  Serial.println(" cm");
  Serial.print("rigthD : ");
  Serial.print(ultraFright.distanceCm() );
  Serial.println(" cm");
  
  //==== 前面 ====//
  if (ultraFforward > maxdis)
  {
    Action(11); //forwardall
    delay(500);
  }//end of if感測器
  else if ((ultraFforward < mindis))
  {
    Action(12); //backwardall
    delay(1000);
  }
  else if ((ultraFforward > mindis) && (ultraFforward < maxdis))
  {
    Action(17); //originall
    delay(1000);
    goto serialUSB;
  ...
  //==== 左邊 ====//
  if ((ultraFleft < mindis))
  {
    Action(13); //rightwardall
    delay(1000);
  }

  //==== 右邊 ====//
  if ((ultraFright < mindis))
  {
    Action(14); //leftwardall
    delay(1000);
  }
...
serialUSB:
  ...
  {
    sequence_number = Serial.read();
    switch (sequence_number)
    {
      case 'f':
        Action(11); //forwardall
        delay(500);
        Serial.println("forwardall.");
        break;
      case 'b':
        Action(12); //backwardall
        delay(500);
        Serial.println("backwardall.");
        break;
      case 's':
        Action(17); //originall，stop
        delay(500);
        Serial.println("stop.");
        break;
      default:
...
int Falling_Task(void) {
  ...
  if ((az) < -0.65) posture_index = 1; //Frontside
  else if ((az) > 0.65) posture_index = 2; // Backside
  else if ((ax) < -0.75) posture_index = 3; // Rightside
  else if ((ax) > 0.75) posture_index = 4; // Leftside
  else if ((az) <= 0.65 && (az) >= -0.65) posture_index = 5; // Stand Status
...
void Packet_Version_Read(void) {
  ...
  Serial.write(type_Crawler);                 //Type
...
void Action(int N) {
  if (N == 0) MusicPlaying_wav_play("none");
    ...
    if (ActionNo_1 != None) XYZrobot.playSeq(ActionNo_1);
  }
  else if (N == 2) {
    MusicPlaying_wav_play(Music_2);
    if (ActionNo_2 != None) XYZrobot.playSeq(ActionNo_2);
  }
  else if (N == 3) {
    MusicPlaying_wav_play(Music_3);
    if (ActionNo_3 != None) XYZrobot.playSeq(ActionNo_3);
  }
  else if (N == 4) {
    MusicPlaying_wav_play(Music_4);
    if (ActionNo_4 != None) XYZrobot.playSeq(ActionNo_4);
  }
  else if (N == 5) {
    MusicPlaying_wav_play(Music_5);
    if (ActionNo_5 != None) XYZrobot.playSeq(ActionNo_5);
  }
  else if (N == 6) {
    MusicPlaying_wav_play(Music_6);
    if (ActionNo_6 != None) XYZrobot.playSeq(ActionNo_6);
  }
  else if (N == 7) {
    MusicPlaying_wav_play(Music_7);
    if (ActionNo_7 != None) XYZrobot.playSeq(ActionNo_7);
  }
  else if (N == 8) {
    MusicPlaying_wav_play(Music_8);
    if (ActionNo_8 != None) XYZrobot.playSeq(ActionNo_8);
  }
  else if (N == 9) {
    MusicPlaying_wav_play(Music_9);
    if (ActionNo_9 != None) XYZrobot.playSeq(ActionNo_9);
  }
  else if (N == 10) {
    MusicPlaying_wav_play(Music_10);
    if (ActionNo_10 != None) XYZrobot.playSeq(ActionNo_10);
  }
  else if (N == 11) {
    MusicPlaying_wav_play(Music_11);
    if (ActionNo_11 != None) XYZrobot.playSeq(ActionNo_11);
  }
  else if (N == 12) {
    MusicPlaying_wav_play(Music_12);
    if (ActionNo_12 != None) XYZrobot.playSeq(ActionNo_12);
  }
  else if (N == 13) {
    MusicPlaying_wav_play(Music_13);
    if (ActionNo_13 != None) XYZrobot.playSeq(ActionNo_13);
  }
  else if (N == 14) {
    MusicPlaying_wav_play(Music_14);
    if (ActionNo_14 != None) XYZrobot.playSeq(ActionNo_14);
  }
  else if (N == 15) {
    MusicPlaying_wav_play(Music_15);
    if (ActionNo_15 != None) XYZrobot.playSeq(ActionNo_15);
  }
  else if (N == 16) {
    MusicPlaying_wav_play(Music_16);
    if (ActionNo_16 != None) XYZrobot.playSeq(ActionNo_16);
  }
  else if (N == 17) {
    MusicPlaying_wav_play(Music_17);
    if (ActionNo_17 != None) XYZrobot.playSeq(ActionNo_17);
  }
  else if (N == 18) {
    MusicPlaying_wav_play(Music_18);
    if (ActionNo_18 != None) XYZrobot.playSeq(ActionNo_18);
  }
  else if (N == 19) {
    MusicPlaying_wav_play(Music_19);
    if (ActionNo_19 != None) XYZrobot.playSeq(ActionNo_19);
  }
  else if (N == 20) {
    MusicPlaying_wav_play(Music_20);
    if (ActionNo_20 != None) XYZrobot.playSeq(ActionNo_20);
  }
  else if (N == 21) {
    MusicPlaying_wav_play(Music_21);
    if (ActionNo_21 != None) XYZrobot.playSeq(ActionNo_21);
  }
  else if (N == 22) {
    MusicPlaying_wav_play(Music_22);
    if (ActionNo_22 != None) XYZrobot.playSeq(ActionNo_22);
  }
  else if (N == 23) {
    MusicPlaying_wav_play(Music_23);
    if (ActionNo_23 != None) XYZrobot.playSeq(ActionNo_23);
  }
  else if (N == 24) {
    MusicPlaying_wav_play(Music_24);
    if (ActionNo_24 != None) XYZrobot.playSeq(ActionNo_24);
  }
  else if (N == 25) {
    MusicPlaying_wav_play(Music_25);
    if (ActionNo_25 != None) XYZrobot.playSeq(ActionNo_25);
  }
  else if (N == 26) {
    MusicPlaying_wav_play(Music_26);
    if (ActionNo_26 != None) XYZrobot.playSeq(ActionNo_26);
  }
  else if (N == 27) {
    MusicPlaying_wav_play(Music_27);
    if (ActionNo_27 != None) XYZrobot.playSeq(ActionNo_27);
  }
  else if (N == 28) {
    MusicPlaying_wav_play(Music_28);
    if (ActionNo_28 != None) XYZrobot.playSeq(ActionNo_28);
  }
  else if (N == 29) {
    MusicPlaying_wav_play(Music_29);
    if (ActionNo_29 != None) XYZrobot.playSeq(ActionNo_29);
  }
  else if (N == 30) {
    MusicPlaying_wav_play(Music_30);
    if (ActionNo_30 != None) XYZrobot.playSeq(ActionNo_30);
  }
  else if (N == 31) {
    MusicPlaying_wav_play(Music_31);
    if (ActionNo_31 != None) XYZrobot.playSeq(ActionNo_31);
  }
  else if (N == 32) {
    MusicPlaying_wav_play(Music_32);
    if (ActionNo_32 != None) XYZrobot.playSeq(ActionNo_32);
  }
  else if (N == 33) {
    MusicPlaying_wav_play(Music_33);
    if (ActionNo_33 != None) XYZrobot.playSeq(ActionNo_33);
  }
  else if (N == 34) {
    MusicPlaying_wav_play(Music_34);
    if (ActionNo_34 != None) XYZrobot.playSeq(ActionNo_34);
  }
  else if (N == 35) {
    MusicPlaying_wav_play(Music_35);
    if (ActionNo_35 != None) XYZrobot.playSeq(ActionNo_35);
  }
  else if (N == 36) {
    MusicPlaying_wav_play(Music_36);
    if (ActionNo_36 != None) XYZrobot.playSeq(ActionNo_36);
  }
  else if (N == 37) {
    MusicPlaying_wav_play(Music_37);
    if (ActionNo_37 != None) XYZrobot.playSeq(ActionNo_37);
  }
  else if (N == 38) {
    MusicPlaying_wav_play(Music_38);
    if (ActionNo_38 != None) XYZrobot.playSeq(ActionNo_38);
  }
  else if (N == 39) {
    MusicPlaying_wav_play(Music_39);
    if (ActionNo_39 != None) XYZrobot.playSeq(ActionNo_39);
  }
  else if (N == 40) {
    MusicPlaying_wav_play(Music_40);
    if (ActionNo_40 != None) XYZrobot.playSeq(ActionNo_40);
  }
  else if (N == 41) {
    MusicPlaying_wav_play(Music_41);
    if (ActionNo_41 != None) XYZrobot.playSeq(ActionNo_41);
  }
  else if (N == 42) {
    MusicPlaying_wav_play(Music_42);
    if (ActionNo_42 != None) XYZrobot.playSeq(ActionNo_42);
  }
  else if (N == 43) {
    MusicPlaying_wav_play(Music_43);
    if (ActionNo_43 != None) XYZrobot.playSeq(ActionNo_43);
  }
  else if (N == 44) {
    MusicPlaying_wav_play(Music_44);
    if (ActionNo_44 != None) XYZrobot.playSeq(ActionNo_44);
  }
  else if (N == 45) {
    MusicPlaying_wav_play(Music_45);
    if (ActionNo_45 != None) XYZrobot.playSeq(ActionNo_45);
  }
  else if (N == 46) {
    MusicPlaying_wav_play(Music_46);
    if (ActionNo_46 != None) XYZrobot.playSeq(ActionNo_46);
  }
  else if (N == 47) {
    MusicPlaying_wav_play(Music_47);
    if (ActionNo_47 != None) XYZrobot.playSeq(ActionNo_47);
  }
  else if (N == 48) {
    MusicPlaying_wav_play(Music_48);
    if (ActionNo_48 != None) XYZrobot.playSeq(ActionNo_48);
  }
  else if (N == 49) {
    MusicPlaying_wav_play(Music_49);
    if (ActionNo_49 != None) XYZrobot.playSeq(ActionNo_49);
  }
  else if (N == 50) {
    MusicPlaying_wav_play(Music_50);
    if (ActionNo_50 != None) XYZrobot.playSeq(ActionNo_50);
  }
  else if (N == 51) {
    MusicPlaying_wav_play(Music_51);
    if (ActionNo_51 != None) XYZrobot.playSeq(ActionNo_51);
  }
  else if (N == 52) {
    MusicPlaying_wav_play(Music_52);
    if (ActionNo_52 != None) XYZrobot.playSeq(ActionNo_52);
  }
  else if (N == 53) {
    MusicPlaying_wav_play(Music_53);
    if (ActionNo_53 != None) XYZrobot.playSeq(ActionNo_53);
  }
  else if (N == 54) {
    MusicPlaying_wav_play(Music_54);
    if (ActionNo_54 != None) XYZrobot.playSeq(ActionNo_54);
  }
...
boolean BT_Packet_Task(void) {
  ...
  if (Serial2.available() >= 7) {
    if ((temp_packet[0] = Serial2.read()) == 0) ; else {
      find_header_BT();
      return false;
    }
...
void BT_ActionEnding(int ActionNum) {
  ...
  delay(50);
  Serial2.write(ActionNum); // Action Number : 1 ~ 0xFF
  delay(50);
}
...
void BT_FW() {
  ...
  delay(50);
  Serial2.write(type_Crawler);              //Type
  delay(50);
